...
func TestItemUsecase_UpdateItem(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			id:   1,
			input: UpdateItemInput{
				Name:          stringPtr("更新後の名前"),
				Brand:         stringPtr("更新後のブランド"),
				PurchasePrice: intPtr(200000),
			},
			setupMock: func(mockRepo *MockItemRepository) {
				existingItem, _ := entity.NewItem("元の名前", "時計", "元のブランド", 100000, "2023-01-01")
				existingItem.ID = 1
				mockRepo.On("FindByID", mock.Anything, int64(1)).Return(existingItem, nil)

				updatedItem := *existingItem
				updatedItem.Name = "更新後の名前"
				updatedItem.Brand = "更新後のブランド"
				updatedItem.PurchasePrice = 200000
				mockRepo.On("Update", mock.Anything, mock.AnythingOfType("*entity.Item")).Return(&updatedItem, nil)
			},
			expectError: false,
		},
		{
			name: "正常系: 一部フィールドのみ更新",
			id:   1,
			input: UpdateItemInput{
				Name: stringPtr("新しい名前"),
			},
			setupMock: func(mockRepo *MockItemRepository) {
				existingItem, _ := entity.NewItem("元の名前", "時計", "ブランド", 100000, "2023-01-01")
				existingItem.ID = 1
				mockRepo.On("FindByID", mock.Anything, int64(1)).Return(existingItem, nil)

				updatedItem := *existingItem
				updatedItem.Name = "新しい名前"
				mockRepo.On("Update", mock.Anything, mock.AnythingOfType("*entity.Item")).Return(&updatedItem, nil)
			},
			expectError: false,
		},
		...
		{
			name: "異常系: 無効な名前（100文字超過）",
			id:   1,
			input: UpdateItemInput{
				Name: stringPtr("この名前は100文字を超えています。この名前は100文字を超えています。この名前は100文字を超えています。この名前は100文字を超えています。この名前は100文字を超えています。"),
			},
			setupMock: func(mockRepo *MockItemRepository) {
				existingItem, _ := entity.NewItem("元の名前", "時計", "ブランド", 100000, "2023-01-01")
				existingItem.ID = 1
